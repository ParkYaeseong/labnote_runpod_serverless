...
2.  `models` 섹션에 아래와 같이 서버 정보를 추가합니다. 파일이 없다면 새로 생성하고 아래 내용을 붙여넣으세요.
    ...
    models:
      # 1. Llama3 70b 모델
      - name: "LabNote AI Server" # Continue UI에 표시될 이름
        provider: openai         # Ollama는 OpenAI와 호환되는 API를 제공합니다.
        model: "biollama3"         # 서버의 .env 파일에 설정된 LLM_MODEL과 일치
        apiBase: "http://<서버_IP_주소>:<11434_외부_포트>/v1" # 1단계에서 확인한 외부 접속 URL
        apiKey: "ollama" #
        title: "LabNote AI"
        
        # 2. BioLlama3 모델 추가
      - name: "LabNote AI (BioLlama3)"
        provider: openai
        model: "biollama3:latest"
        apiBase: "http://<서버_IP_주소>:<11434_외부_포트>/v1"
        apiKey: "ollama"
        title: "VESSL BioLlama3"
