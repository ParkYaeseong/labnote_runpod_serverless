...
## 5. 서버리스 배포 (Docker & RunPod)

서버리스 아키텍처의 배포는 Docker 이미지를 빌드하여 Docker Hub에 푸시하고, RunPod이 해당 이미지를 사용하여 워커를 실행하는 방식으로 이루어집니다.

### A. Docker 이미지 구조

빌드 효율성을 위해 2단계 Docker 빌드 전략을 사용합니다.

- **`Dockerfile.base`**: OS, CUDA, Python, Redis, Ollama 등 자주 변경되지 않는 무거운 의존성을 설치하여 `labnote-ai-base`라는 베이스 이미지를 생성합니다. 이 이미지는 한 번 빌드해두면 다시 빌드할 필요가 거의 없습니다.
- **`Dockerfile`**: 베이스 이미지를 기반으로, 자주 변경되는 애플리케이션 소스 코드(`labnote-ai-backend`)를 Git 저장소에서 직접 클론하여 최종 `labnote-ai-app` 이미지를 생성합니다. 이 방식 덕분에 코드 변경 시 매우 빠른 빌드가 가능합니다.

### B. 모델 관리 전략

LLM 모델은 크기가 매우 크기 때문에 Docker 이미지에 포함하지 않습니다. 대신 다음과 같은 전략을 사용합니다.

- **RunPod 네트워크 볼륨**: `labnote-ai`라는 이름의 네트워크 볼륨을 생성하고, RunPod 워커 설정에서 이 볼륨을 컨테이너의 `/runpod-volume/ollama_models` 경로에 마운트합니다.
- **동적 모델 다운로드**: 컨테이너 시작 시 실행되는 `start.sh` 스크립트가 마운트된 볼륨을 확인하여 필요한 모델이 없으면 `ollama pull` 명령으로 자동으로 다운로드합니다. 다운로드된 모델은 네트워크 볼륨에 저장되므로, 워커가 재시작되어도 모델을 다시 받을 필요가 없습니다.

### C. 빌드 및 배포 자동화

- **`build_and_push.sh`**: 이 셸 스크립트는 `Dockerfile.base`와 `Dockerfile`을 순차적으로 빌드하고, 버전 태그를 붙여 Docker Hub에 푸시하는 전체 과정을 자동화합니다.
- **`start.sh`**: 컨테이너의 Entrypoint 역할을 하는 스크립트입니다. Redis, Ollama 서버를 시작하고, B에서 설명한 모델 다운로드 로직을 수행한 후, 마지막으로 `runpod_handler.py`를 실행하여 서버리스 워커를 활성화합니다.

이러한 구조를 통해 코드 변경에 따른 배포는 몇 분 내로 완료되며, 무거운 모델 파일은 영속적인 네트워크 저장소에서 효율적으로 관리됩니다.
