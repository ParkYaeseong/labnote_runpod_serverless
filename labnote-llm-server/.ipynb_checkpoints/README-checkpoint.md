...
## 🛠️ 최근 업데이트 (2025-10-08)

- `deploy_model.sh`가 환경 변수 `LLAMA_CPP_PATH`를 기준으로 llama.cpp 최신 구조를 자동 탐지합니다. 하이픈/언더스코어가 다른 변환 스크립트(`convert-hf-to-gguf.py` vs `convert_hf_to_gguf.py`)와 `llama-quantize` 바이너리를 모두 지원합니다.
- 학습 산출물이 `labnote-ai-backend/biollama3-v2-dpo`에 위치한다는 점을 반영해 배포 경로 fallback을 정리했습니다.
- 네트워크 없이도 배포 스크립트가 실행되도록 `stubs/mistral_common`을 추가했습니다. 실제 Mistral 토크나이저 기능이 필요한 경우 정식 `mistral_common` 패키지를 설치해야 합니다.
- llama.cpp 스크립트가 SentencePiece 의존성을 찾지 못하면 자동으로 Hugging Face BPE 경로로 전환하도록 패치해 Llama 3 계열 모델이 변환 중단 없이 처리됩니다.
- GGUF 산출물은 `labnote-llm-server/gguf_models`에 기록됩니다. 배포 스크립트를 실행하는 계정이 해당 디렉터리에 쓰기 권한을 가지고 있는지 확인하세요.
