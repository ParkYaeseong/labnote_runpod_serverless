...
if [ -z "${LLAMA_CPP_PATH}" ]; then
    echo "❌ LLAMA_CPP_PATH 환경 변수가 설정되어 있지 않습니다. .env 파일 또는 셸 환경에서 설정해주세요."
    exit 1
fi
...
TRAINED_MODEL_DIR="${TRAINED_MODEL_DIR:-../labnote-ai-backend/biollama3-v2-dpo}"
if [ ! -d "${TRAINED_MODEL_DIR}" ]; then
    ALT_TRAINED_MODEL_DIR="../biollama3-v2-dpo"
    if [ -d "${ALT_TRAINED_MODEL_DIR}" ]; then
        ...
        echo "   run_full_dpo_pipeline.sh 실행 전 DPO 학습이 성공적으로 완료되었는지 확인하세요."
        exit 1
    fi
fi
...
# (!) 일부 Ollama 버전은 허용 문자 집합이 제한되어 있으므로 기본값은 소문자/숫자/하이픈만 사용합니다.
RAW_OLLAMA_MODEL_NAME="${NEW_OLLAMA_MODEL_NAME:-biollama3-dpo-v2}"
